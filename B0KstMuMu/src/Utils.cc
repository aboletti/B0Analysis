...
RooAbsPdf* Utils::ReadRTEffPDF (unsigned int q2BinIndx, RooRealVar* z, RooRealVar* y,RooRealVar* p)
{
  ...
  myString << (DirEfficiency != "" ? MakeAnalysisPATH(DirEfficiency).c_str() : "") << "effKEpdf_out_RT.root";
  std::cout << "[Utils::ReadRTEffPDF]\tReading 2D binned efficiency file : " << myString.str().c_str() << std::endl;
  ...
  RooArgSet* set=EffPDFm->getVariables();
  set->find("ctK")->SetName("z");
  set->find("ctL")->SetName("y");
  set->find("phi")->SetName("p");

  // TH3* h3 = (TH3*) EffPDFm ->createHistogram("ctK,ctL,phi");
  // RooDataHist* EffPDF = new RooDataHist("Eff","Eff",RooArgList(*z,*y,*p),h3);
  RooAbsPdf* EffPDF = new RooHistPdf("EffPDF","EffPDF",RooArgSet(*z,*y,*p),EffPDFm->dataHist(),1);
  return EffPDF;
}
...
RooAbsPdf* Utils::ReadWTEffPDF (unsigned int q2BinIndx, RooRealVar* z, RooRealVar* y,RooRealVar* p)
{
  ...
  //#######################
  std::stringstream myString;
  myString << (DirEfficiency != "" ? MakeAnalysisPATH(DirEfficiency).c_str() : "") << "effKEpdf_out_WT.root";
  std::cout << "[Utils::ReadWTEffPDF]\tReading 2D binned efficiency file : " << myString.str().c_str() << std::endl;
  ...
  // RooAbsPdf* Eff = new RooDataHist("Eff","Eff",RooArgList(*z,*y,*p),h3);
  RooAbsPdf* EffPDF = new RooHistPdf("EffPDF","EffPDF",RooArgSet(*z,*y,*p),EffPDFm->dataHist(),1);
...
TH2D* Utils::Get2DEffHistoButPhi (std::string histoName, std::vector<double>* q2Bins, std::vector<double>* cosThetaKBins, std::vector<double>* cosThetaLBins, std::vector<double>* phiBins, unsigned int q2Indx, effStruct myEff)
{
  double Eff    = 0.0;
  double EffErr = 0.0;
...
TH2D* Utils::Get2DEffHistoButCosThetaK (std::string histoName, std::vector<double>* q2Bins, std::vector<double>* cosThetaKBins, std::vector<double>* cosThetaLBins, std::vector<double>* phiBins, unsigned int q2Indx, effStruct myEff)
{
  double Eff    = 0.0;
  double EffErr = 0.0;
  ...
  TH2D* Histo = new TH2D(histoName.c_str(), histoName.c_str(), cosThetaLBins->size()-1, cosThetaLBins_, phiBins->size()-1, phiBins_);
  Histo->SetXTitle("cos(#theta#lower[-0.4]{_{#font[12]{l}}})");
  Histo->GetXaxis()->SetTitleOffset(1.8);
  Histo->SetYTitle("#phi");
  Histo->GetYaxis()->SetTitleOffset(1.8);
  Histo->SetZTitle("Efficiency");
  ...
  // # Read binned efficiency #
  // ##########################
  for (unsigned int j = 0; j < cosThetaLBins->size()-1; j++)
    for (unsigned int k = 0; k < phiBins->size()-1; k++)
      {
	IntegrateEffCosThetaK(q2Bins,cosThetaKBins,cosThetaLBins,phiBins,q2Bins->operator[](q2Indx),phiBins->operator[](k),cosThetaLBins->operator[](j),myEff,&Eff,&EffErr);
...
TH2D* Utils::Get2DEffHistoButCosThetaL (std::string histoName, std::vector<double>* q2Bins, std::vector<double>* cosThetaKBins, std::vector<double>* cosThetaLBins, std::vector<double>* phiBins, unsigned int q2Indx, effStruct myEff)
{
  double Eff    = 0.0;
  double EffErr = 0.0;
  double* cosThetaKBins_ = MakeBinning(cosThetaKBins);
  ...
  TH2D* Histo = new TH2D(histoName.c_str(), histoName.c_str(), cosThetaKBins->size()-1, cosThetaKBins_, phiBins->size()-1, phiBins_);
  Histo->SetXTitle("cos(#theta#lower[-0.4]{_{#font[122]{K}}})");
  Histo->GetXaxis()->SetTitleOffset(1.8);
  Histo->SetYTitle("#phi");
  Histo->GetYaxis()->SetTitleOffset(1.8);
  ...
  for (unsigned int j = 0; j < cosThetaKBins->size()-1; j++)
    for (unsigned int k = 0; k < phiBins->size()-1; k++)
      {
	IntegrateEffCosThetaL(q2Bins,cosThetaKBins,cosThetaLBins,phiBins,q2Bins->operator[](q2Indx),phiBins->operator[](k),cosThetaKBins->operator[](j),myEff,&Eff,&EffErr);
  ...
  if ((mumuq2Indx != -1) && (cosThetaKBinIndx != -1) && (cosThetaLBinIndx != -1))
    {
      ...
      for (unsigned int l = 0; l < phiBins->size()-1; l++)
	{
	  int xBin = l*(cosThetaLBins->size()-1)*(cosThetaKBins->size()-1)*(q2Bins->size()-1) +
	    cosThetaLBinIndx*(cosThetaKBins->size()-1)*(q2Bins->size()-1) +
	    cosThetaKBinIndx*(q2Bins->size()-1) +
	    mumuq2Indx;

	  myEffVal.Num1 = myEffVal.Num1 + myEff.Num1[xBin];
	  myEffVal.Num2 = myEffVal.Num2 + myEff.Num2[xBin];
	  myEffVal.Den1 = myEffVal.Den1 + myEff.Den1[xBin];
	  myEffVal.Den2 = myEffVal.Den2 + myEff.Den2[xBin];

	  myEffVal.Err2PoisNum1 = myEffVal.Err2PoisNum1 + myEff.Err2PoisNum1[xBin];
	  myEffVal.Err2PoisNum2 = myEffVal.Err2PoisNum2 + myEff.Err2PoisNum2[xBin];
	  myEffVal.Err2PoisDen1 = myEffVal.Err2PoisDen1 + myEff.Err2PoisDen1[xBin];
	  myEffVal.Err2PoisDen2 = myEffVal.Err2PoisDen2 + myEff.Err2PoisDen2[xBin];

	  if (myEff.Err2PoisNum1[xBin] > 0.0)
	    myEffValOrg.Num1 = myEffValOrg.Num1 + pow(myEff.Num1[xBin],2.0) /
	      myEff.Err2PoisNum1[xBin];
	    
	  if (myEff.Err2PoisNum2[xBin] > 0.0)
	    myEffValOrg.Num2 = myEffValOrg.Num2 + pow(myEff.Num2[xBin],2.0) /
	      myEff.Err2PoisNum2[xBin];

	  if (myEff.Err2PoisDen1[xBin] > 0.0)
	    myEffValOrg.Den1 = myEffValOrg.Den1 + pow(myEff.Den1[xBin],2.0) /
	      myEff.Err2PoisDen1[xBin];
	    
	  if (myEff.Err2PoisDen2[xBin] > 0.0)
	    myEffValOrg.Den2 = myEffValOrg.Den2 + pow(myEff.Den2[xBin],2.0) /
	      myEff.Err2PoisDen2[xBin];

	  myEffVal.Err2WeigNum1 = myEffVal.Err2WeigNum1 + myEff.Err2WeigNum1[xBin];
	  myEffVal.Err2WeigNum2 = myEffVal.Err2WeigNum2 + myEff.Err2WeigNum2[xBin];
	  myEffVal.Err2WeigDen1 = myEffVal.Err2WeigDen1 + myEff.Err2WeigDen1[xBin];
	  myEffVal.Err2WeigDen2 = myEffVal.Err2WeigDen2 + myEff.Err2WeigDen2[xBin];
	}
...
bool Utils::IntegrateEffCosThetaL (std::vector<double>* q2Bins, std::vector<double>* cosThetaKBins, std::vector<double>* cosThetaLBins, std::vector<double>* phiBins, double mumuMass2, double phi, double cosThetaK, effStruct myEff, double* Eff, double* EffErr)
{
  int mumuq2Indx;
  int phiBinIndx;
  int cosThetaKBinIndx;
  ...
  mumuq2Indx       = SearchBin(mumuMass2,q2Bins);
  phiBinIndx       = SearchBin(phi,phiBins);
  cosThetaKBinIndx = SearchBin(cosThetaK,cosThetaKBins);

  if ((mumuq2Indx != -1) && (phiBinIndx != -1) && (cosThetaKBinIndx != -1))
    {
      mumuq2Indx       = SearchBin(mumuMass2,q2Bins);
      phiBinIndx       = SearchBin(phi,phiBins);
      cosThetaKBinIndx = SearchBin(cosThetaK,cosThetaKBins);

      for (unsigned int l = 0; l < cosThetaLBins->size()-1; l++)
	{
	  int xBin = phiBinIndx*(cosThetaLBins->size()-1)*(cosThetaKBins->size()-1)*(q2Bins->size()-1) +
	    l*(cosThetaKBins->size()-1)*(q2Bins->size()-1) +
	    cosThetaKBinIndx*(q2Bins->size()-1) +
	    mumuq2Indx;
...
bool Utils::IntegrateEffCosThetaK (std::vector<double>* q2Bins, std::vector<double>* cosThetaKBins, std::vector<double>* cosThetaLBins, std::vector<double>* phiBins, double mumuMass2, double phi, double cosThetaMu, effStruct myEff, double* Eff, double* EffErr)
{
  int mumuq2Indx;
  int phiBinIndx;
  int cosThetaLBinIndx;
  ...
  if ((mumuq2Indx != -1) && (phiBinIndx != -1) && (cosThetaLBinIndx != -1))
    {
      mumuq2Indx       = SearchBin(mumuMass2,q2Bins);
      phiBinIndx       = SearchBin(phi,phiBins);
      cosThetaLBinIndx = SearchBin(cosThetaMu,cosThetaLBins);

      for (unsigned int l = 0; l < cosThetaKBins->size()-1; l++)
	{
	  int xBin = phiBinIndx*(cosThetaLBins->size()-1)*(cosThetaKBins->size()-1)*(q2Bins->size()-1) +
	    cosThetaLBinIndx*(cosThetaKBins->size()-1)*(q2Bins->size()-1) +
	    l*(q2Bins->size()-1) +
